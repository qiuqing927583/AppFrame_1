...
import com.android.yawei.jhoa.facescan.faceuntil.ImageUtil;
                ...
                APIService.getInstance().identify(new OnResultListener<RegResult>() {
                    ...
                    public void onResult(RegResult result) {
                        ...
                        if (result == null) {
                            mUploading = false;
                            if (mDetectCount >= 3) {
                                Toast.makeText(DetectLoginActivity.this, "人脸校验不通过,请确认是否已注册/绑定", Toast.LENGTH_SHORT).show();
                                finish();
                            }
                            return;
                        }
