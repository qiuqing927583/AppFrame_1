...
//        sendMessage(message, status, true);
        ...
        if(speechEndTime>0){
            long diffTime = System.currentTimeMillis() - speechEndTime;
            bean.setDiffTime("说话结束到识别结束耗时【" + diffTime + "ms】");
        }
        speechEndTime = 0;
        bean.setSuccess(results[0]);
        bean.setSuccessJson(recogResult.getOrigalJson());
        String voiceJson = JSON.toJSONString(bean);
        sendMessage(voiceJson, status, true);
    }
    @Override
    public void onAsrVolume(int volumePercent, int volume){
        Log.i(TAG, "音量百分比" + volumePercent + " ; 音量" + volume);
    }
    //语音识别失败，或者按住没有说话等错误信息返回
    @Override
...
//        speechEndTime = 0;
        VoiceBean bean  = new VoiceBean();
        bean.setErrorCode("错误码："+errorCode+","+subErrorCode);
        if (speechEndTime > 0) {
            long diffTime = System.currentTimeMillis() - speechEndTime;
            bean.setDiffTime("说话结束到识别结束耗时【" + diffTime + "ms】");
        }
        bean.setError("错误消息:" + errorMessage + "；描述信息：" + descMessage);
        String voiceJson = JSON.toJSONString(bean);
        speechEndTime = 0;
        sendMessage(voiceJson, status, true);
        speechEndTime = 0;
